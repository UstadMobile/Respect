...
object RespectFileUtil {
    ...
    fun parseURLQueryString(urlQuery: String): Map<String, String> {
        ...
        return parsedParams.map { RespectURLEncoder.decodeUTF8(it.key) to RespectURLEncoder.decodeUTF8(it.value) }.toMap()
    }
    ...
    fun mapToQueryString(ht: Map<String, String?>): String {
        ...
        while (keys.hasNext()) {
            ...
            key = keys.next()
            sb.append(RespectURLEncoder.encodeUTF8(key)).append('=')
            sb.append(RespectURLEncoder.encodeUTF8(ht[key] as String))
        }
