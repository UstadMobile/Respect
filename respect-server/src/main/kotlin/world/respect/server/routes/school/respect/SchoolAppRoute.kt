...
fun Route.SchoolAppRoute(
    schoolDataSource: (ApplicationCall) -> SchoolDataSource = { call ->
        call.requireAccountScope().get()
    },
) {

    get(SchoolAppDataSource.ENDPOINT_NAME) {
        call.response.header(HttpHeaders.Vary, HttpHeaders.Authorization)

        call.respondOffsetLimitPaging<SchoolApp>(
            params = call.request.queryParameters.offsetLimitPagingLoadParams(),
            pagingSource = schoolDataSource(call).schoolAppDataSource.listAsPagingSource(
                loadParams = DataLoadParams(),
                params = SchoolAppDataSource.GetListParams.fromParams(
    ...
    post(SchoolAppDataSource.ENDPOINT_NAME) {
        schoolDataSource(call).schoolAppDataSource.store(
            list = call.receive()
        )
